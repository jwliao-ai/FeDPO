...
def concatenated_inputs(
    ...
    for k in batch:
        if k.startswith('rejected') and isinstance(batch[k], torch.Tensor):
            ...
            concatenated_batch[concatenated_key] = torch.cat((concatenated_batch[concatenated_key],pad_to_length(batch[k], max_length, pad_value=pad_value),),dim=0)
...
class TensorParallelTrainer(BasicTrainer):
    ...
    def __init__(self,
        ...
        """
        super().__init__(batch_counter, example_counter, decay, logger_dir, client_idx, policy, config, seed, run_dir, reference_model, rank,
                         world_size)
